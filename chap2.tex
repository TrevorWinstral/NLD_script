...
\end{align}
From here, we see that the determinate is equal $\lambda^2 + 1 = 0$, yielding the roots $\lambda_{1,2}= \pm i$, i.e. the fixed point is \underline{not} hyperbolic. Thus the linearized dynamics is inconclusive for the nonlinear system. In this particular case, $q$ turns out to be stable, which can be concluded from another approach (see later).
\end{ex}

In the last example, the importance of hyperbolicity was not eccentuated, as the latter fixed point had the same stability type in the linearized system as in the full system. This leads us to question if there are cases where the stability type between the linear and nonlinear systems is not preserved.
...
\begin{ex}[Criticality of hyperbolicity in Hartman-Grobman]
	...
	\end{align}
	We have a single root $\lambda_1 = 0$, hence $x=0$ is a nonhyperbolic fixed point. Disregarding this fact, we may be inclined to conclude that $x=0$ is a stable fixed point. This is not the case, as we can see by analyzing the full nonlinear dynamics for $a>0$ as in Fig. \ref{fig:hyperbol_counter_ex}, where we observe that $x=0$ is an unstable fixed point.
\begin{figure}[h!]
	\centering
...
\begin{align}
	...
	\end{vmatrix}
\end{align}
Then we have that if for all $i$ $D_i >0$ then $ \textrm{Re} \lambda_i <0$ for all $i$.

Further if for each $i$ $ \textrm{Re} \lambda _i<0$ then $a_i >0$ for all $i$ (a necessary condition). Therefore if there exists an $a_k<0$, we know immediately that the fixed point cannot be asymptotically stable.
\end{theorem}
\begin{remark}[]
...
\begin{ex}[Applying Routh-Hurwitz criterion]
...
\begin{ex}[Watt's centrifugal governer for steam engines]
...
\begin{figure}[h!]
	...
	\label{fig:governor}
\end{figure}
The two masses (of mass $\frac{m}{2}$) rotate counter clockwise, and their position in radians is given by $\theta$, with their rotational velocity $\dot{\theta}$. The masses are attached by a rod of length $L$ and their deflection from the vertical position is measured by $\varphi$. Thus smaller $\dot{\theta}$ allowed for an increase in steam supply.
...
\begin{align}
	m \dot{\varphi} = -b \dot{\varphi} - m \left( \frac{g}{L} - n^2 w^2 \cos(\varphi) \right) \sin(\varphi).
\end{align}
...
\end{align}
If $\sin(\varphi_0)=0$, we have an unphysical state and ignore this case. We also nondimensionalize for simplification and set $L=1$. Now we linearize at the fixed point $x_0$ 
\begin{align}
	\dot{y} = Ay; \quad A = Df(x_0) =
	\begin{pmatrix}
		0 & 1 & 0 \\
		n^2 \omega^2 \cos^2(\varphi_0) & -\frac{b}{m} & n^2 \omega _0 \sin^2(\varphi_0) \\
...
\begin{align}
	\underbrace{1}_{a_3} \lambda^3 + \underbrace{\frac{b}{m}}_{a_2}\lambda ^2 + \underbrace{ \frac{g}{L}\frac{\sin^2(\varphi_0)}{\cos(\varphi_0)}}_{a_1} \lambda + \underbrace{ \frac{g}{L} \frac{2k \sin^2 (\varphi_0)}{J \omega_0}}_{a_0} = 0.
\end{align}
Now check the Routh-Hurwitz criterion for asymptotic stability:
\begin{enumerate}
	\item The necessary condition for $ \textrm{Re} \lambda _k<0$ for all $k$: $a_j> 0$ for all $ $ is fulfilled.
	\item Next we check the subdeterminants
\begin{align}
	D_0 &= a_0 = \frac{g}{L} \frac{2k \sin^2 (\varphi_0)}{J \omega_0}>0;\quad \\ 
	D_1 &= a_1 = \frac{g}{L}\frac{\sin^2 (\varphi_0)}{\cos(\varphi_0)} >0; \\
	D_2 &= 
	\begin{vmatrix}
		a_1 & a_0 \\
		a_3 & a_2
	\end{vmatrix}
	= a_1a_2 - a_0a_3 = \frac{b}{m}\frac{g}{L}\frac{\sin^2(\varphi_0)}{\cos(\varphi_0)} - \frac{g}{L}\frac{2k \sin^2(\varphi_0)}{J \omega_0} >0;\\
...
\end{align}
From the equation for fixed points we know 
\begin{align*}
...
\begin{figure}[h!]
	...
	\caption{Change in the eigenvector configuration as small modification are made to the Watt engine governor.}
	\label{fig:watt_eigv}
\end{figure}
\end{ex}
\newpage
...
\begin{theorem}[]
	Consider
	\begin{align}
		\dot{x} = f(x), \quad f\in C^1,\quad x \in \mathbb{R}^{n};\quad f(x_0) = 0.
	\end{align}
	Assume that there exists a $V:U \to \mathbb{R}$ with $V\in C^1(U)$ for $U \subset \mathbb{R}^{n}$ open and $x_0 \in U$ which fulfills the following
	\begin{enumerate}
		\item $V$ is positive definite in a neighborhood of $x_0$, i.e.
			\begin{align}
				V(x_0)=0;\quad V(x)>0,\ x\in U\setminus \{ x_0 \}.
			\end{align}
		\item $\dot{V}$ is negative semidefinite in the same neighborhood, i.e.
			\begin{align}
				\dot{V} = \langle DV(x), f(x) \rangle \leq 0,\ x\in U.
			\end{align}
	\end{enumerate}
	Then $x=x_0$ is Lyapunov stable. $V$ is called a Lyapunov function. The hypotheses are illustrated in Fig. \ref{fig:lyap_thm_assumptions}.	
\end{theorem}
\begin{figure}[h!]
	\centering
...
\begin{proof}
	First choose $\epsilon > 0$ and define $\alpha(\epsilon)= \min_{x\in \partial B_\epsilon(x_0)} V(x) > 0$. Note that $\alpha(\epsilon)$ is well defined as $V\in C^{0}(U)$ and $\partial B_{\epsilon}(x)$ is compact and spherical for small enough $\epsilon$. There exists an $x^{*} \in \partial B_{\epsilon}(x)$ with $V(x^{*}) = \alpha(\epsilon) \leq V(x)$ for all $x\in \delta B_{\epsilon}(x)$. Next, define $U_{\epsilon}= \{ x\in B_{\epsilon}:\ V(x) < \alpha(\epsilon)  \ $. Notice that $x_0 \in V_{\epsilon}$ implies that $V_\epsilon$ is nonempty and open, thus $U_{\epsilon} \cap \partial B_{\epsilon}(x_ 0) $ is empty. Therefore there exists a ball $B_{\delta(\epsilon)} \subset U_{\epsilon} $ which contains $x_0$. This can be seen in Fig. \ref{fig:lyap_pf_1}.
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.45\textwidth]{figures/ch2/22lyap_pf_1.png}
	\caption{The constellation of $U$, $U_\epsilon$, $\partial B_{\epsilon}(x_0)$, and $B_{\delta(\epsilon)}(x_0)$ from the proof of Lyapunov's direct method. The red arrow points at $B_{\delta(\epsilon)}(x_0)$ and the yellow arrow at a connected component of $U_\epsilon$.}
	\label{fig:lyap_pf_1}
\end{figure}

Now observe that for every $\tilde{x}_{0} \in B_{\delta(\epsilon)}(x_0)$ we have that $V(x(t; \tilde{x}_{0})) \leq V(\tilde{x}_{0} < \alpha(\epsilon))$. The first inequality comes from hypothesis (ii) and the second inequality from the definition of $U_{\epsilon}$. This implies that for $x(t; \tilde{x}_0) \in U_{\epsilon}$ we have that $x(t; \tilde{x}_{0})$ is not in $\partial B_{\epsilon}(x_0)$ for any $t> 0$. Hence $x(t; \tilde{x}_0)$ must stay in the ball $B_{\epsilon}(x_0)$ for all times $t> 0$ by continuity. 
\end{proof}
...
\begin{remark}[]
	In each of these theorems, the definiteness of $ \dot{V}$ can be replaced by semidefiniteness, if we add that the set $\{x\in U:\ \dot{V}(x)=0 \}$ does not contain full trajectories of the system. This is called Krasovki's condition.
\end{remark}

Now we would like to put these theorems into practice with a few examples.
\begin{ex}[Stability analysis of pendulum with Lyapunov's direct method]
...
\end{align}
Transforming the dynamical system to be an ODE we obtain
\begin{align}
	x = 
	\begin{pmatrix}
		x_1 \\x_2
...
\begin{align}
	E(0,0) = 0;\quad DE(0,0) = 0I;\quad D^2(0,0) =
\begin{pmatrix}
	mgl & 0 \\
	0 & ml^2
\end{pmatrix}.
...
\begin{align}
	...
	= 0.	
\end{align}
Thus $\dot{E}$ is negative semidefinite. Now the hypotheses of Theorem 1 are fulfilled, hence $x=0$ is Lyapunov stable. Importantly, this is a nonlinear result and we never had to reduce the system!

At the fixed point $x = (\pi, 0)^T$. We check again using the energy. First we realize that $E(\pi, 0)= 2mgl$, so we subtract the constant and redefine to obtain $\tilde{E}= E - 2mgl$. Now $E(\pi, 0) = 0$, although this is not essential. Next we calculate
\begin{align}
	DE(\pi, 0) = 0I;\quad D^2E(\pi, 0) = 
	\begin{pmatrix}
		-mgl & 0 \\
		0 & ml^2
	\end{pmatrix}
	.
\end{align}
Hence, $E$ is indefinite at $(\pi, 0)$. Next we find $\dot{E}(\pi, 0) = 0$, i.e. $\dot{E}$ is semidefinite. We cannot apply Theorem 4, but linearization already concluded that $(\pi,0)$ was unstable.
\end{ex}
 \begin{ex}[Stability analysis of friction pendulum]
...
\end{align}
This is a conserved quantity in all mechanics problems, but generally cannot be calculated without already knowing the trajectories. Here, however, we can
\begin{align}
	V(x) = \frac{1}{2} ml x_0^2 + mgl(1- \cos(x_1)) - T_0(\underbrace{x_1}_{\varphi(t)} - \underbrace{x_1(0)}_{\varphi(0)}).
\end{align}
...
\end{pmatrix}.
\end{align}
The Hessian is positive definite as long as $\overline{x}_1 \in \left(-\frac{\pi }{2}, \frac{\pi }{2}\right)$, and all fixed points in this region are stable by Theorem 1. The Hessian is indefinite if $\overline{x}_1 \not \in \left[-\frac{\pi }{2}, \frac{\pi }{2}\right]$, but in this case Theorem 4 is not applicable as $\dot{V}$ is not definite.
 \end{ex}
