...
Following changes in the design, the systems suddenly became unstable. To address this Vishnegradky studied the root caus in 1877. We first derive the equation of motion. For the governor we use the equation of motion for a rotation hoop (with viscous damping coefficient $b$).
\begin{align}
	mL^2 \ddot{\varphi} + bL^2\left( \frac{g}{L} - \dot{\theta}^2 \cos(\varphi) \right) \sin(\varphi) = 0;\quad b>0.
\end{align}
Next, let $\omega $ denote the angular velocity of the steam engine, i.e. with the gear ratio $n$ $\dot{\theta} = n \omega$. Then we find
\begin{align}
