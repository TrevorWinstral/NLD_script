...
.
\end{align}
{\color{blue} left matrix was using amsmath, right using array. we can choose which we want}
...
\section{Consequences of area preservation for 2D time-periodic Hamiltonians}
...
\end{align}
An example of such a system is the periodically forced pendulum with the Hamiltonian  {\color{blue} I am not sure what his $x_2$ is here, it looks like $P\varphi$ }
\begin{align}
...
\begin{align}
	1 = \det\left(DP_{t_0}(x_0)\right) = \lambda_1 \lambda^2.
\end{align}
...
\end{enumerate}
{\color{blue} what about $\lambda_1 = re^{i\alpha}$ and $\lambda_2 = r'e^{-i\alpha }$ for $r>1$ and $r r'=1$? This should be a possible configuration which also has product equal to 1.}
