...
The orientation preserving nature of $P_{\varepsilon}$ can be seen by using Liouville's Theorem 
	\begin{align}
		\det(DF_{t_0}^{t}(p)) = \exp \left( \int_{t_0}^{t_0+T}  \textrm{div}  ( f(x(s;t_0,p),s))ds\right),
	\end{align}
...
\end{align}
For a zero to exist, $R^{0}(\omega)$ cannot exceed the value $\frac{\gamma }{\delta}$, as $\sin $ is bounded by $1$. Therefore when we have $R^{0}(\omega) \leq \frac{\gamma }{\delta }$, a root of the Melnikov function exists. This root is nontransient only when it is quadratic, i.e. when only a peak of the $\sin $ function intersects with $R^{0}(\omega) $. This occurs exactly when $R ^{0}(\omega)=\frac{\gamma }{\delta}$. In all other cases, the $\sin $ wave intersects the curve of $R^{0}(\omega) $ transiently, i.e. the zero is transient. Thiis is illustrated in Fig. \ref{fig:beam_tangle}, where equality  occurs along the graph of $R^{0}$ shows the set on which nontransient zeros exist, the area above this curve contains all of the point for which the zero is transient.
\begin{figure}[h!]
	\centering
