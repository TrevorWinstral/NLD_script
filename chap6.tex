\chapter{Introduction to chaotic dynamics}
...
\begin{exercise}
Show that the number of $k$-periodic orbits for the Bernoulli shift map on two symbols is
\begin{align}
	N(k) = \frac{1}{k}\left(2^k - \sum_{\langle i,k \rangle }^{} i N(i)\right),		
\end{align}
where $\langle i,k\rangle$ means that the integer $i$ divides the integer $k$.
\end{exercise}
...
\begin{exercise}
Show that any two periodic orbits of the Bernoulli shift map are connected by infinitely many heteroclinic orbits.
\end{exercise}
...
\begin{proposition}[] \label{prop:bernoulli_transitive}
...
\begin{proposition}[Sensitive dependence on initial conditions] \label{prop:bernoulli_sensitive}
